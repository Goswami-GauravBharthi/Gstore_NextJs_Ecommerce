    ...
    const fetchSellerStatus = async () => {
        ...
        try {
            ...
            if (["approved", "rejected","pending"].includes(data.status)) {
                ...
                switch (data.status) {
                    case "approved":
                        setMessage("Your store has approved you can start selling now");
                        setTimeout(() => router.push("/store"), 5000);
                        break;
                    case "rejected":
                        setMessage("Your store has rejected , you can contact to addmin for more details..");
                        break;
                    case "pending":
                        setMessage("Your store requested is pending, please wait for approval");
                        break;
                    default:
                        break;
                }
        ...
        return (
            <div className="min-h-[80vh] mx-6 flex items-center justify-center text-slate-400">
                <div className="flex flex-col items-center gap-5">
                    <h1 className="text-2xl sm:text-4xl font-semibold">Please login to continue</h1>
                    <button onClick={openSignIn} className="bg-slate-800 text-white px-12 py-2 rounded mt-10 mb-40 active:scale-95 hover:bg-slate-900 transition cursor-pointer">Login</button>
                </div>
            </div>
        )
    }
                ...
                <div className="mx-6 min-h-[70vh] my-16">
                    <form onSubmit={e => toast.promise(onSubmitHandler(e), { loading: "Submitting data..." })} className="max-w-7xl mx-auto flex flex-col items-start gap-3 text-slate-500">
                        ...
                        <div>
                            ...
                            <p className="max-w-lg">To become a seller on Gstore, submit your store details for review. Your store will be activated after admin verification.</p>
                        </div>
