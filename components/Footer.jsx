...
const Footer = () => {
    ...
    const linkSections = [
        ...
        },
        {
            title: "CONTACT",
            links: [
                { text: "9512993362", path: '/', icon: MailIcon },
                { text: "gaurav02105@gmail.com", path: '/', icon: PhoneIcon },
                { text: "Bhavnagar", path: '/', icon: MapPinIcon }
            ]
        }
    ];
    ...
    return (
        <footer className="w-full bg-surface border-t border-secondary/10">
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-16 border-b border-secondary/10 text-text-muted">
                    <div>
                        ...
                        <div className="flex items-center gap-3 mt-8">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} className="flex items-center justify-center w-10 h-10 bg-background text-primary hover:bg-primary hover:text-white hover:scale-105 hover:shadow-md transition rounded-full border border-secondary/10">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-8 text-sm">
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-medium text-text-main md:mb-6 mb-4 tracking-wide">{section.title}</h3>
                                <ul className="space-y-3">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2 group">
                                            {link.icon && <span className="text-secondary group-hover:text-primary transition-colors"><link.icon /></span>}
                                            <Link href={link.path} className="hover:text-primary transition-colors">{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <p className="py-6 text-sm text-center md:text-left text-text-muted/80">
                    Copyright 2025 © gstore. Crafted with care.
                </p>
            </div>
        </footer>
    );
